...
int main(int argc, char **argv)
{
    ...
    if (!loaded)
    {
        ...
        try
        {
            ...
            std::cout << "indexed " << b.documents << " docs (" << b.unique_terms << " terms, " << b.postings
            ...
            if (skipped)
                std::cout << "skipped" << "unusable files\n";
            if (b.documents == 0)
                std::cerr << "warning: no documents found under " << data_dir << "\n";
            if (!index_path.empty())
            {
                ...
                if (r.ok)
                    ...
                    std::cerr << "warning: could not save index: " << r.error << "\n";
            }
        }
    ...
    se::ApiHandler api(engine, se::ApiConfig{data_dir, index_path}),
        se::HttpServer server(http_threads, http_threads * 64, [&api](const se::HttpRequest &r)
                              { return api.handle(r); });
    api.set_http_stats_source([&server]
                              { return server.stats(); });
